...
describe('ABD #3 — strict-majority quorum behavior', () => {
  it('requires a distinct-replica majority before read phase 2 starts', () => {
    ...
    deliverFirst((m) => m.msg.type === 'ReadPhase1Response' && m.from === 3);
    ...
    // A duplicate from the same replica must not manufacture quorum.
    const heldResponse = h.outbox.findIndex((m) => m.msg.type === 'ReadPhase1Response' && m.from === 3);
    expect(heldResponse).toBe(-1);
    // Recreate a duplicate safely by delivering another query to node 3,
    // then duplicating its response before consuming it.
    deliverFirst((m) => m.msg.type === 'ReadPhase1Query' && m.to === 4);
    const response4 = h.outbox.findIndex((m) => m.msg.type === 'ReadPhase1Response' && m.from === 4);
    expect(response4).toBeGreaterThanOrEqual(0);
    h.duplicate(response4);
    expect(h.outbox.filter((m) => m.msg.type === 'ReadPhase2WriteBack')).toHaveLength(0);

    // The second distinct remote response makes 3/5 and must start phase 2.
    h.deliver(response4);
